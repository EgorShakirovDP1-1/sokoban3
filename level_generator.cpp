...
void place_player() {
    ...
    int index = dis(gen);
    int x = available_spaces[index].first;
...
bool can_reach_all_targets(int x, int y, std::vector<std::pair<int, int>> targets) {
    ...
    while (!frontier.empty()) {
        ...
        if (matrix[current.first - 1][current.second] != '#' && std::find(visited.begin(), visited.end(), std::make_pair(current.first - 1, current.second)) == visited.end()) {
            frontier.push_back(std::make_pair(current.first, current.second));
        }
    }
}